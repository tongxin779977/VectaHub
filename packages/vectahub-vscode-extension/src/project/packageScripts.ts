...
function readPackageJson(workspaceFolder: string): PackageJson | null {
...
export function detectPackageTasks(workspaceFolder: string, pm: PackageManagerType, preParsedPkg?: PackageJson): ProjectTask[] {
...
export function getAllPackageScripts(workspaceFolder: string, pm: PackageManagerType, preParsedPkg?: PackageJson): ProjectTask[] {
  const pkg = preParsedPkg || readPackageJson(workspaceFolder);
  if (!pkg) return [];
