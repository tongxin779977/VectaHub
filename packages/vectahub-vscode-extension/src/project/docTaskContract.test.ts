...
import { deriveDocExcerptForTask } from './docTaskContract.js';
...
function buildHash(input: { docContent: string; taskId: string; label: string }) {
  const excerpt = deriveDocExcerptForTask(input);
...
describe('doc task instruction hash excerpt symmetry', () => {
  it('任务片段变化时 hash 应变化（即使文档开头不变）', () => {
    const baseHead = '# 项目文档\n\n前言保持不变。\n\n';
    const taskId = 'P2-1';
    const label = '实现合同边界';
    ...
    expect(buildHash({ docContent: before, taskId, label })).not.toBe(
      buildHash({ docContent: after, taskId, label }),
    );
  });

  it('仅文档开头变化时 hash 不应变化（任务片段不变）', () => {
    const taskId = 'P2-1';
    const label = '实现合同边界';
    ...
    expect(buildHash({ docContent: before, taskId, label })).toBe(
      buildHash({ docContent: after, taskId, label }),
    );
  });
});
