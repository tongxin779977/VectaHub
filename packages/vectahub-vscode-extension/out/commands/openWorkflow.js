...
exports.registerOpenWorkflowCommand = registerOpenWorkflowCommand;
const vscode = __importStar(require("vscode"));
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
function registerOpenWorkflowCommand(context) {
    const disposable = vscode.commands.registerCommand('vectahubTasks.openWorkflow', async () => {
        // 尝试在当前工作区找 .vectahub/workflows
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder)
            return;
        const workflowsDir = path_1.default.join(workspaceFolder.uri.fsPath, '.vectahub', 'workflows');
        if (!fs_1.default.existsSync(workflowsDir)) {
            vscode.window.showInformationMessage('未在本工作区找到 .vectahub/workflows 目录。');
            return;
        }
        const files = fs_1.default.readdirSync(workflowsDir).filter(f => f.endsWith('.yaml') || f.endsWith('.yml'));
        if (files.length === 0) {
        ...
        if (selected) {
            const doc = await vscode.workspace.openTextDocument(path_1.default.join(workflowsDir, selected));
