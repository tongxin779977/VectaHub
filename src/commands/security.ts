  ...
  securityCmd
    ...
    .action(async (command: string, options: { cliTool?: string; json?: boolean }) => {
      ...
      if (result.isDangerous) {
        ...
        auditHelper.securityAlert(result.rule?.id || 'unknown', command, result.severity || 'unknown', sessionId);
        auditHelper.securityAction('test_command', command, 'DANGEROUS', sessionId);
      } else {
        output.push(`\n✅ SAFE`);
        auditHelper.securityAction('test_command', command, 'SAFE', sessionId);
      }
      output.push('');
