...
describe('WorkflowTemplate', () => {
  let TEST_DIR: string;
  beforeEach(() => {
    TEST_DIR = mkdtempSync(join(tmpdir(), 'vectahub-template-'));
  });
  ...
  describe('loadTemplate', () => {
    ...
    it('should throw for non-existent file', () => {
      expect(() => loadTemplate(environment, '/nonexistent/template.yaml')).toThrow();
    });
  });
  ...
  describe('listTemplates', () => {
    ...
    it('should return empty array for non-existent directory', () => {
      const templates = listTemplates(environment, '/nonexistent/dir');
      expect(templates.length).toBe(0);
    });
    ...
    it('should filter by category', () => {
      ...
      const gitTemplates = listTemplates(environment, TEST_DIR, 'git');
    ...
    it('should filter by tag', () => {
      writeFileSync(join(TEST_DIR, 'git.yaml'), SAMPLE_TEMPLATE);
      writeFileSync(join(TEST_DIR, 'backup.yaml'), SAMPLE_TEMPLATE_NO_PARAMS);
      const commitTemplates = listTemplates(environment, TEST_DIR, undefined, 'commit');
  ...
  describe('instantiateTemplate', () => {
    it('should instantiate template with default params', () => {
      const path = join(TEST_DIR, 'git-commit.yaml');
      writeFileSync(path, SAMPLE_TEMPLATE);
      const workflow = instantiateTemplate(environment, path, {});
    ...
    it('should substitute parameters into steps', () => {
      const path = join(TEST_DIR, 'git-commit.yaml');
      writeFileSync(path, SAMPLE_TEMPLATE);
      const workflow = instantiateTemplate(environment, path, { message: 'my commit' });
    ...
    it('should use custom parameter values', () => {
      const path = join(TEST_DIR, 'git-commit.yaml');
      writeFileSync(path, SAMPLE_TEMPLATE);
      const workflow = instantiateTemplate(environment, path, { files: 'src/', message: 'update' });
...
`;
      ...
      expect(() => instantiateTemplate(environment, path, {})).toThrow(/target/);
    });
