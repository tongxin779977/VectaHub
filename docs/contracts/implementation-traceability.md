...
| Templates and scheduling | [Templates generation scheduling](./templates-generation-scheduling.md) | Template and generation command paths where implemented | Template generation tests and command checks | Scheduling behavior must be described as target design unless implementation evidence exists. | Migration Contract |
| Service import/export | [Service import/export](./service-import-export.md) | Local service, daemon, and import/export command paths where implemented | Service command smoke checks and data migration checks | Daemon/service capabilities require explicit runtime verification. | Partial Implementation |
