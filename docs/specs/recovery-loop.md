...
`run-task` 的完整执行合同、`needs_confirmation` 双来源、`dry-run` 权威语义、完成边界和“未收口执行”术语，以 [Run-Task 执行合同规格](./run-task-execution-contract.md) 为准。
...
如果 `run-task` 最终以 `timeout` 收口，但 `gitChanges` 已存在，则仍必须返回这些 `gitChanges`，同时保持 `verification` 缺失；恢复判断必须把这种情况视为“未收口执行”，即已有副作用但未正常完成。
...
原因：
- 已经发生仓库副作用，但 CLI 没有正常收口；这属于“未收口执行”，应基于当前 diff 和失败上下文继续处理，而不是盲重试。
