...
failed_system_internal:
  ...
  - Agent 输出明确说明“本地命令工具无法启动”“无法读取代码”“未能执行代码修改”等环境阻塞
  - Agent 输出使用等价措辞，例如“当前任务被工具层阻断”“当前被环境阻塞”“本地命令/文件访问工具不可用”“未做代码改动”“本次实际修改文件：无”“无法进入工作区”
  - 即使 Agent 子进程退出码为 `0`，只要命中上述软失败且没有实际改动，也必须归类到 `failed_system_internal`
